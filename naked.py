...
def push_asteroids_arrays_to_db(request_day, ast_array, hazardous): # nosuta datus uz masivu
	for asteroid in ast_array:
		if mysql_check_if_ast_exists_in_db(request_day, asteroid[9]) == 0: # parbauda datus
			...
			logger.debug("Asteroid already IN DB") # ja ir jau datu baaze
...
if __name__ == "__main__":
	...
	if r.status_code == 200:
		...
		logger.debug("Hazardous asteorids: " + str(len(ast_hazardous)) + " | Safe asteroids: " + str(len(ast_safe)))
		...
		if len(ast_hazardous) > 0:
			...
			push_asteroids_arrays_to_db(request_date, ast_hazardous, 1) # ja masiva garums ir lielaaks par 0. 
		else:
